...
ベクトル $\boldsymbol{x}$ の長さをノルムといい、$\|x\|$ であらわします。長さ（ノルム）が１のベクトルを単位と言います。
0でない二つのベクトル、$\boldsymbol{a}$、$\boldsymbol{b}$ の始点を繋げベクトル間の角度が90°の場合、内積は必ず0になります。このとき $\boldsymbol{a}$, $\boldsymbol{b}$ は直交すると言います。
...
=A^{-1}$ と表します。
行列$A$、ベクトル $\boldsymbol{x}$ 、$λ$($\neq0$))とすると$A\boldsymbol{x}＝λ\boldsymbol{x}$ が成り立つ場合、$λ$を固有値、$\boldsymbol{x}$ を固有ベクトルと言います。
...
$B^{t}AB$ が対角行列とは、すなわち対角化のことであるから$B^{t}=B^{-1}$ ならばBはもとめる直交行列である。よって
$$
\vec{v_{1}}=
\begin{pmatrix}
1 \\
